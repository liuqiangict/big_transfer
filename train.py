...
def mktrainval(args, logger):
    ...
    if args.dataset == "cifar10":
        ...
        #train_set = tv.datasets.ImageNet(args.datadir, transform=val_tx, train=True, download=True)
        #valid_set = tv.datasets.ImageNet(args.datadir, transform=val_tx, train=False, download=True)
    else:
...
def run_eval(model, data_loader, device, chrono, logger, writer, step):
    ...
    writer.add_scalar('Eval/loss', np.mean(all_c), step)
    writer.add_scalar('Eval/accuracy_top1', np.mean(all_top1), step)
    writer.add_scalar('Eval/accuracy_top5', np.mean(all_top5), step)
    model.train()
